...
class NerfModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        h3 = self.fc_layers_group1(x) 
        
        h4 = F.relu(self.layer_4(h3)) 
        
        h5_in = torch.cat([h4, h3], dim=-1)
        
        out = self.fc_layers_group2(h5_in)
...
def get_rays(height: int, width: int, intrinsics: torch.Tensor, tform_cam2world: torch.Tensor) \
    ...
    y_vals = torch.arange(height, device=device, dtype=torch.float32)
    x_vals = torch.arange(width, device=device, dtype=torch.float32)
    ...
    pixel_coords_homo = torch.stack([x_flat, y_flat, ones], dim=0) 

    if intrinsics.dtype != pixel_coords_homo.dtype:
        intrinsics = intrinsics.to(pixel_coords_homo.dtype)
    
    inv_k = torch.inverse(intrinsics)
    cam_coords = inv_k @ pixel_coords_homo
    
    R = tform_cam2world[:3, :3]
    if R.dtype != cam_coords.dtype:
        R = R.to(cam_coords.dtype)
        
    ray_dirs_world = R @ cam_coords
    
    ray_directions = ray_dirs_world.T.reshape(height, width, 3)
    
    # ray_directions = ray_directions / torch.norm(ray_directions, dim=-1, keepdim=True)
    
    cam_origin = tform_cam2world[:3, 3]
    ray_origins = cam_origin.expand(height, width, 3)
...
def sample_points_from_rays(
    ...
    step_size = (far_thresh - near_thresh) / num_samples
    
    t_vals = torch.linspace(near_thresh, far_thresh - step_size, num_samples, device=device)
    
    depth_values = t_vals.expand(height, width, num_samples)
    
    if randomize:
        mids = torch.rand((height, width, num_samples), device=device) * step_size
        depth_values = depth_values + mids
    else:
        pass
    
    ray_o = ray_origins.unsqueeze(2)
    ray_d = ray_directions.unsqueeze(2)
    z_vals = depth_values.unsqueeze(3)
    
    query_points = ray_o + ray_d * z_vals
...
def compute_compositing_weights(sigma: torch.Tensor, depth_values: torch.Tensor) -> torch.Tensor:
    ...
    last_dist = torch.tensor([1e10], device=depth_values.device).expand(dists[..., :1].shape)
...
def render_image_nerf(height: int, width: int, intrinsics: torch.tensor, tform_cam2world: torch.tensor,
    ...
    batches = get_minibatches(flat_points, chunksize=1024*32)
    rgb_preds = []
    sigma_preds = []
    
    for batch in batches:
        encoded_batch = encoding_function(batch)
        rgb, sigma = model(encoded_batch)
        
        rgb_preds.append(rgb)
        sigma_preds.append(sigma)
        
    flat_rgb = torch.cat(rgb_preds, dim=0) 
    flat_sigma = torch.cat(sigma_preds, dim=0)
    
    sigma = flat_sigma.reshape(height, width, depth_samples_per_ray)
    rgb = flat_rgb.reshape(height, width, depth_samples_per_ray, 3)
    
    weights = compute_compositing_weights(sigma, depth_values)
    
    rgb_predicted = torch.sum(weights.unsqueeze(-1) * rgb, dim=2) 
    
    depth_predicted = torch.sum(weights * depth_values, dim=2)
